...
git -C "$WORK" add -A && git -C "$WORK" -c user.name=excalidrop -c user.email=excalidrop@local commit -m "excalidrop: publish viewer" --allow-empty && git -C "$WORK" push origin excalidrop --force
...
# Point Pages at the excalidrop branch (PUT works whether Pages is on or off)
CUR="$(gh api "repos/${REPO}/pages" --jq '.source.branch' 2>/dev/null || echo none)"
if [ "$CUR" = "none" ]; then
  gh api "repos/${REPO}/pages" -X POST -f build_type=legacy -f 'source[branch]=excalidrop' -f 'source[path]=/' >/dev/null
elif [ "$CUR" != "excalidrop" ]; then
  gh api "repos/${REPO}/pages" -X PUT -f build_type=legacy -f 'source[branch]=excalidrop' -f 'source[path]=/' >/dev/null
fi
